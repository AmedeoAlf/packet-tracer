...
export function SideBar({
  ...
  const resizeBarId = "sidebarResizeHandle" + (rightSide ? "R" : "L");
  ...
  useEffect(() => {
    const mousedown = (ev: MouseEvent) => {
      ...
      if (ev.target.id != resizeBarId) return;
      setResizing(true);
    };
    document.addEventListener("mousedown", mousedown);
    return () => document.removeEventListener("mousedown", mousedown);
  }, [setResizing, resizeBarId]);

  return (
    <>
      <div
        className={
          `transition fixed top-12.5 h-(--h-spec-cont) ${rightSide ? "right" : "left"}-0 p-3 pointer-events-none ` +
          (children && open
            ? ""
            : rightSide
              ? "translate-x-full"
      ...
      >
        <div className="bg-sidebar p-4 border-zinc-500 border-2 w-full pl-5 rounded-xl pointer-events-auto overflow-y-auto h-max max-h-full relative">
          <div
            className={`absolute top-0 ${rightSide ? "left" : "right"}-0 h-full py-4`}
          >
            <div
              id={resizeBarId}
      ...
      <button
        className={
          `transition fixed top-12.5 right-0 w-6 h-(--h-spec-cont)
            bg-radial-[at_100%_50%] from-sky-700 to-transparent to-60%
            hover:to-80% ` + (children ? "" : "translate-x-10")
        }
